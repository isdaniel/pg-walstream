...
pub enum ColumnValue {
    /// SQL NULL
    Null,
    /// Text value from PostgreSQL (UTF-8 encoded string).
    /// Uses [`Bytes`] for zero-copy from the protocol buffer.
    Text(Bytes),
    /// Binary data from PostgreSQL (bytea or binary-mode columns).
    /// Stored as raw bytes.
    Binary(Bytes),
}
...
impl RowData {
    ...
    ///
    /// Format: `[2-byte column count]` then for each column:
    /// `[2-byte name length][name bytes][ColumnValue encoding]`.
    pub fn encode(&self, buf: &mut BytesMut) {
