...
mod tests {
    ...
    fn test_pg_md5_password_pattern() {
        // Simulate md5(password + user)
        let mut inner_data = Vec::new();
        inner_data.extend_from_slice(b"test");
        inner_data.extend_from_slice(b"test");
        let inner = compute(&inner_data);
        let inner_hex = hex_digest(&inner);

        // Simulate md5(inner_hex + salt)
        let salt = [0u8; 4];
        let mut outer_data = Vec::new();
        outer_data.extend_from_slice(inner_hex.as_bytes());
        outer_data.extend_from_slice(&salt);
        let outer = compute(&outer_data);
